...
if kekel == "y" or kekel =="Y":
	...
	if kekel == "Y":
		...
		things = kekel.split(",")
	for port in things: 
		yeet.run(["ufw deny " +port],shell=True)
		print("Port #" + port + " has been blocked :)")
		yeet.run(['echo "port #' + port + ' is done heker" >> log.txt'],shell=True)
	kekel = input("Do you want to save changes across reboot? (Y/N)")
	if kekel == "Y":
		yeet.run(["service iptables save"],shell=True)
...
if kekel == "y" or kekel == "Y":
	...
	for service in things:
		yeet.run(["systemctl stop " +service],shell=True)
		yeet.run(["systemctl disable " +service],shell=True)
		yeet.run(["systemctl daemon-reload"],shell=True)
		yeet.run(["systemctl reset-failed"],shell=True)
